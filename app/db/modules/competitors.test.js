...
const { competitors, conn } = require('../index');
...
const uids = chance.n(() => `github:${chance.natural({ min: 100000, max: 2000000 })}`, 5);
...
describe('games management', () => {
  ...
  it('feed db', async () => {
    const samples = R.pipe(
      R.xprod,
      R.map(([gid, uid]) => ({ gid, uid, club: chance.natural({ min: 1, max: 500 }) })),
  ...
  it('can delete one entry', async () => {
    ...
    const list = await competitors.delete({ uid, gid }).then(() => competitors.find({ uid, gid }));
