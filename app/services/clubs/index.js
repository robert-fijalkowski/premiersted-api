...
const useAllKeysToFilter = (query) => {
  ...
  const comparator = name =>
    R.ifElse(
      R.is(String),
      R.pipe(lowerCased, R.contains(`${R.prop(name, query)}`.toLocaleLowerCase())),
      R.equals(parseFloat(R.prop(name, query))),
    );
  const foundPartials = name => R.pipe(R.prop(name), comparator(name));
...
module.exports = {
  get(query = {}, dataSet = data) {
    const filterData = R.cond([
      [
        R.both(R.prop('id'), nConditions(1)),
        ({ id }) => R.pipe(R.filter(R.propEq('id', id)), R.head),
      ],
    ...
    return R.ifElse(R.is(Array), R.map(R.omit(['keywords'])), R.omit(['keywords']))(results);
  },
